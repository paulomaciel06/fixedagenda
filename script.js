...
function handleWhatsAppClick(event) {
    ...
    // Usa window.location.href para dispositivos móveis
    if (/Android|iPhone/i.test(navigator.userAgent)) {
        // Abre no app do WhatsApp sem redirecionar a página original
        setTimeout(() => {
            const whatsappUrl = `whatsapp://send?phone=seu-numero&text=${message}`;
            window.location.href = whatsappUrl;
        }, 100);
    } else {
        // Para desktop mantém o comportamento de nova aba
        window.open(`https://wa.me/seu-numero?text=${message}`, '_blank');
    }
    
    // Previne qualquer redirecionamento
    return false;
}
